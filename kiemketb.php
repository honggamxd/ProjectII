...
$idkk = $_SESSION['idkk'];
global $conn;
$sql = "SELECT * FROM kiemke WHERE idkk = '$idkk'";
$result = mysqli_query($conn, $sql);
$kk = mysqli_fetch_assoc($result);
$diadiem = $kk['diadiem'];
$date = $kk['date'];
$values = getTBbyPhong($diadiem);
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    foreach ($values as $key => $tb) {
        $Namsd = isset($_POST['Namsd'.$key]) ? $_POST['Namsd'.$key] : null;
        $Trangthai = isset($_POST['Trangthai'.$key]) ? $_POST['Trangthai'.$key] : null;
        $Ghichu = isset($_POST['ghichu'.$key]) ? $_POST['ghichu'.$key] : null;
        $ketquakk = isset($_POST['ketquakk'.$key]) ? $_POST['ketquakk'.$key] : null;
        $idtb = $tb['idthietbi'];

        global $conn;
        $sql = "INSERT INTO kiemke_tb(idkk,idTB,ketquakk) VALUE ('$idkk','$idtb','$ketquakk')";
        $result = mysqli_query($conn, $sql);
        $str = "Thanh lý";
        $str1 = "Cần sửa chữa";
        if ($ketquakk === $str) {
            $sql = "UPDATE thietbi SET status ='0' WHERE idthietbi = '$idtb'";
            $result = mysqli_query($conn, $sql);
        } else if ($ketquakk === $str1) {
            $sql = "UPDATE thietbi SET Trangthai ='$Trangthai', Namsd ='$Namsd',Ghichu ='$ketquakk' WHERE idthietbi = '$idtb'";
            $result = mysqli_query($conn, $sql);
        } else {
            $sql = "UPDATE thietbi SET Trangthai ='$Trangthai', Namsd ='$Namsd', Ghichu='$Ghichu' WHERE idthietbi = '$idtb'";
            $result = mysqli_query($conn, $sql);
        }
    }
                        ...
                        <div class="row">
                            ...
                            <div class="col-lg-7">
                                ...
                                <div class="text-center">
                                    <h3>Kiểm kê</h3>
                                    <h5>Địa điểm : <?php echo $diadiem ?> </h5>
                                    <h5>Ngày : <?php echo $date ?> </h5>
                                </div>
                                    ...
                                    <table class="table table-bordered dataTable" id="dataTable" width="100%"
                                        ...
                                        <tr role="row">
                                            <th class="sorting_asc" tabindex="0" aria-controls="dataTable" rowspan="1"
                                                ...
                                                style="width: 57px;">Tên
                                            </th>
                                            <th class="sorting" tabindex="0" aria-controls="dataTable" rowspan="1"
                                                colspan="1" aria-label="Office: activate to sort column ascending"
                                                style="width: 50px;">Năm sd
                                            </th>
                                            <th class="sorting" tabindex="0" aria-controls="dataTable" rowspan="1"
                                                colspan="1" aria-label="Age: activate to sort column ascending"
                                                style="width: 31px;">Trạng thái
                                            </th>
                                            <th class="sorting" tabindex="0" aria-controls="dataTable" rowspan="1"
                                                colspan="1" aria-label="Age: activate to sort column ascending"
                                                style="width: 31px;">Ghi chú
                                            </th>
                                            <th class="sorting" tabindex="0" aria-controls="dataTable" rowspan="1"
                                                colspan="1" aria-label="Age: activate to sort column ascending"
                                                style="width: 31px;">Kết quả kiểm kê
                                            </th>
                                        ...
                                        <tr role="row" class="odd">
                                            ...
                                            <td>
                                                <div class="form-group">
                                                    <input type="text" value="<?php echo $tb["Namsd"] ?>"
                                                           class="form-control" id="exampleInputPassword1"
                                                           name="Namsd<?php echo $key ?>" placeholder="">
                                                </div>
                                            </td>
                                            <td>
                                                <div class="form-group">
                                                    <input type="text" class="form-control" id="exampleInputPassword1"
                                                           name="Trangthai<?php echo $key ?>" value="<?php echo $tb["Trangthai"] ?>"
                                                           placeholder="">
                                                </div>
                                            </td>
                                            <td>
                                                <div class="form-group">
                                                    <input type="text" class="form-control" id="exampleInputPassword1"
                                                           name="ghichu<?php echo $key ?>" value="<?php echo $tb["Ghichu"] ?>"
                                                           placeholder="">
                                                </div>
                                            </td>
                                            <td>
                                                <div class="form-group">
                                                    ...
                                                    <label><select name="ketquakk<?php echo $key ?>" aria-controls="dataTable"
